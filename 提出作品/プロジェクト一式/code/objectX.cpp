...
void CObjectX::Draw()
{
	...
	{
		// 向きを反映
		D3DXMatrixRotationYawPitchRoll(&mtxRot, m_gModelData[nCntObj].rot.y, m_gModelData[nCntObj].rot.x, m_gModelData[nCntObj].rot.z);
		D3DXMatrixMultiply(&m_mtxWorld, &m_mtxWorld, &mtxRot);

		// 位置を反映
		D3DXMatrixTranslation(&mtxTrans, m_gModelData[nCntObj].pos.x, m_gModelData[nCntObj].pos.y, m_gModelData[nCntObj].pos.z);
		D3DXMatrixMultiply(&m_mtxWorld, &m_mtxWorld, &mtxTrans);
